...
class CoffeeShopAppTest {
    ...
    class CommandTests {
        ...
        void multipleUndoes() {
            ...
            invoker.undoLastCommand(); // undo FULFILLED → READY
            assertEquals(OrderStatus.READY, order.getStatus());

            invoker.undoLastCommand(); // undo READY → PLACED
            assertEquals(OrderStatus.PLACED, order.getStatus());

            invoker.undoLastCommand(); // undo PLACED → CANCELLED
            assertEquals(OrderStatus.CANCELLED, order.getStatus());
        }
